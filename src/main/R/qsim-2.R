...
# --- Compare total blocking_recv duration for 1 vs 4 routing threads ---
get_blocking_totals <- function(routing_threads) {
  path <- sprintf("output/v6.4/1pct-12-router-%d/instrument", routing_threads)
  files <- list.files(path, pattern = "^instrument_process_\\d+\\.csv$", full.names = TRUE)
  data <- map_dfr(files, function(f) {
    rank_num <- str_extract(basename(f), "(?<=instrument_process_)\\d+")
    read_csv(f, show_col_types = FALSE) %>% mutate(rank = as.integer(rank_num))
  })
  data %>%
    filter(func_name == "blocking_recv") %>%
    group_by(rank) %>%
    summarise(total_duration_ns = sum(duration, na.rm = TRUE), .groups = "drop") %>%
    mutate(routing_threads = routing_threads)
}

blocking_1 <- get_blocking_totals(1)
blocking_4 <- get_blocking_totals(4)

combined_blocking <- bind_rows(blocking_1, blocking_4)

ggplot(combined_blocking, aes(x = factor(rank), y = total_duration_ns / 1e6, fill = factor(routing_threads))) +
  geom_col(position = "dodge") +
  scale_y_continuous(labels = scales::comma) +
  labs(
    title = "Total blocking_recv Duration per Rank for 1 vs 4 Routing Threads",
    x = "Rank",
    y = "Total Duration (ms)",
    fill = "Routing Threads"
  ) +
  theme_minimal(base_size = 24)